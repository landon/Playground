...
namespace GraphRecognizer
{
    public class EquivalenceRelation<T>
    {
        ...
        public T FindRepresentative(T x)
        {
            return Find(LookupNode(x)).T;
        }

        public IEnumerable<T> EnumerateEquivalenceClass(T x)
        {
            var nx = LookupNode(x);
            var xRoot = Find(nx);
        ...
        public IEnumerable<IEnumerable<T>> EnumerateEquivalenceClasses()
        {
